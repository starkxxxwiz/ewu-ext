...
(function () {
  ...
  var CONFIG = {
    ...
    LOG_PREFIX: '[EWU Buddy]',
    ROUTINE_LOG: '[EWU Buddy][Routine]',
  ...
  var ALLOWED_PATHS = [
    ...
    { path: '/home/advisingoffline',       id: 'advisingOffline', label: 'Advising Offline' },
    { path: '/EWU-TEST/AdvisingOnlinePageSource.html', id: 'advising', label: 'Advising Page' },
    { path: '/ewu-test/advisingonlinepagesource.html', id: 'advising', label: 'Advising Page' }
  ];
  ...
  function detectPage() {
    ...
    if (pnl.indexOf('advisingonlinepagesource') !== -1 || (pnl.indexOf('advising') !== -1 && pnl.indexOf('.html') !== -1)) {
      return { id: 'advising', label: 'Advising Page' };
    }
    ...
    _exportCoursesPDF: async function () {
      ...
      try {
        ...
        function drawFooter() {
          ...
          pdf.text('Page ' + pageNum + '  |  EWU Buddy v' + CONFIG.VERSION, pw / 2, ph - 4, { align: 'center' });
        }
  ...
  var AdvisingTableEnhancerModule = {
    ...
    init: function (settings) {
      ...
      var isAdvPage = (pn.indexOf('/home/advising') !== -1) ||
                      ...
                      (pn.indexOf('advisingonlinepagesource') !== -1) ||
    ...
    _handleData: function (data) {
      ...
      var totalCourses = courseSet.size || items.length;
      debugLog('ADV: ' + totalCourses + ' courses (' + items.length + ' sections) captured');
      Toast.show('Advising course data loaded (' + totalCourses + ' courses)', 'success', 2500);
      this._enhanceTables();
      this._applyFilters();
    },
    ...
    _enhanceTables: function () {
      ...
      for (var ti = 0; ti < tables.length; ti++) {
        ...
        // Body rows enhancement (never overwrite tbody.innerHTML so Angular ng-repeat bindings remain intact)
        ...
        for (var ri = 0; ri < rows.length; ri++) {
          ...
          var sectionId = '';
          if (apiItem && apiItem.SectionId) {
            sectionId = String(apiItem.SectionId);
          } else if (tr.getAttribute('data-section-id')) {
            sectionId = tr.getAttribute('data-section-id');
          }
          ...
          if (sectionId) tr.setAttribute('data-section-id', sectionId);
    ...
    _applyFilters: function () {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        if (show) {
          visibleRows++;
          var cCode = tr.getAttribute('data-code');
          if (cCode) visibleCourses.add(cCode);
        }
      }
      this._updateCount(visibleCourses.size);
    },

    _updateCount: function (count) {
      var el = safeQuery('#ewu-adv-stats');
      if (el) el.textContent = 'Courses: ' + count;
    },
    ...
    _exportAdvisingPDF: async function () {
      ...
      try {
        ...
        function drawFooter() {
          pdf.setFontSize(7); pdf.setTextColor(160, 160, 160); pdf.setFont('helvetica', 'normal');
          pdf.text('Page ' + pageNum + '  |  EWU Buddy v' + CONFIG.VERSION + ' (Advising Export)', pw / 2, ph - 4, { align: 'center' });
        }
  ...
  var RecommendedCourseModule = {
    ...
    _exportPDF: async function () {
      ...
      try {
        ...
        function drawFooter() {
          pdf.setFontSize(7); pdf.setTextColor(160, 160, 160); pdf.setFont('helvetica', 'normal');
          pdf.text('Page ' + pageNum + '  |  EWU Recommended Courses Report  |  EWU Buddy v' + CONFIG.VERSION, pw / 2, ph - 4, { align: 'center' });
        }
  ...
  async function main() {
    log('EWU Buddy v' + CONFIG.VERSION + ' starting...');
